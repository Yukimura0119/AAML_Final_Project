...
inline void Softmax(const SoftmaxParams &params,
	...
	using FixedPointScaledDiff = gemmlowp::FixedPoint<int32_t, 5>;
	using FixedPointAccum = gemmlowp::FixedPoint<int32_t, 12>;
	using FixedPoint0 = gemmlowp::FixedPoint<int32_t, 0>;
	InputT max_in_row = -128;
	for (int c = 0; c < 10; ++c) {
	...
	for (int c = 0; c < 10; ++c) {
		int32_t input_diff = static_cast<int32_t>(input_data[c]) - max_in_row;
		if (input_diff >= -124) {
			...
			sum_of_exps =
				sum_of_exps +
				gemmlowp::Rescale<12>(exp_on_negative_values(scaled_diff_f8));
		}
	}

	int num_bits_over_unit;
	FixedPoint0 shifted_scale = FixedPoint0::FromRaw(
		GetReciprocal(sum_of_exps.raw(), 12, &num_bits_over_unit));
	for (int c = 0; c < 10; ++c) {
		int32_t input_diff = static_cast<int32_t>(input_data[c]) - max_in_row;
		if (input_diff >= -124) {
			const int32_t input_diff_rescaled =
			...
			int32_t unsat_output = gemmlowp::RoundingDivideByPOT(
				(shifted_scale * exp_in_0).raw(), num_bits_over_unit + 23);
			const int32_t shifted_output =
				unsat_output + static_cast<int32_t>(-128);
			output_data[c] = static_cast<OutputT>(
...
inline void SoftmaxInt16(const SoftmaxParams &params,
	...
	for (int i = 0; i < outer_size; ++i) {
		...
		// This loops computes the exp values and their sum. We will need the
		// exp values later on in the function so we cache them in the
		// output_data buffer. This is an optimization done to avoid calculating
		// the exp values twice making use of the output_data buffer as scratch
		// memory.
		int32_t sum_of_exps = 0; // Q16.15 fixed point format.
		int16_t *exp_results_Q015 = output_data + i * depth;
		for (int c = 0; c < depth; ++c) {
			exp_results_Q015[c] = SoftMaxCalculateExp(params, input_data, depth,
													  max_in_row, i, c);
		...
		int32_t shifted_sum =
			((static_cast<int64_t>(sum_of_exps) << (headroom_plus_one - 1)) +
			 (1 << 13)) >>
			14;
		// since the LUT computes 1/(1 + x) we need to first compute x = (sum -
		// 1). also, the LUT expects a symmetrical input, so we must also
		// recenter x from [0, 65535] to [-32768, 32767].
