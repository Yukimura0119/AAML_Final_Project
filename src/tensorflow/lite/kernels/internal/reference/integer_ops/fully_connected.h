...
// accum_depth:64
inline void
FullyConnected(const FullyConnectedParams &params,
	...
	for (int out_c = 0; out_c < 10; ++out_c) {
		...
		// 	int32_t filter_val = filter_data[out_c * 64 + d];
		// 	acc += filter_val * (input_val + 128);
		// }

		int32_t acc = cfu_op0(3, 0, 0);
		for (int d = 0; d < 64; d += 4) {
			uint32_t input_val = *((uint32_t *)(input_data + d));
			uint32_t filter_val =
				*((uint32_t *)(filter_data + (out_c << 6) + d));
			acc = cfu_op0(4, input_val, filter_val);
		}
		acc += bias_data[out_c];
...
FullyConnected(const FullyConnectedParams &params,
			   ...
			   const RuntimeShape &filter_shape, const int8_t *filter_data,
			   const RuntimeShape &bias_shape, const AccumScalar *bias_data,
