...
ConvPerChannel(const ConvParams &params, const int32_t *output_multiplier,
	...
	for (int m = 0; m < pad_M; m += tile_size) {
		for (int k = 0; k < pad_K; k += tile_size) {
			...
			for (int n = 0; n < pad_N; n += (tile_size << 1)) {
				// int idx = 0;
				...
				for (int i = 0; i < tile_size; i += 4) {
					for (int j = 0; j < tile_size; j += 4) {
						for (int r = 0; r < 4; r++) {
							...
							if (n + j + r + tile_size < pad_N) {
								data1 = *(
									(uint32_t *)(im2col[n + j + r + tile_size] +
												 m + i));
								data2 = *(
									(uint32_t *)(kernel[n + j + r + tile_size] +
												 k + i));
							}
				...
				for (int i = 0; i < 16; i++) {
					for (int j = 0; j < 16; j++) {
						int32_t data = cfu_op0(4, ((j >> 2) << 4) + i, j & 0x3);
						matmul[i][j] += data;
						__asm volatile("nop");
						data = cfu_op0(5, ((j >> 2) << 4) + i, j & 0x3);
						matmul[i][j] += data;
					}
				}
			}
